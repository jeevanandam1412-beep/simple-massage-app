...
export const RealtimeThread: React.FC = () => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-[var(--border-subtle)] bg-[var(--bg-main)] transition-colors">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-subtle)] rounded-2xl p-5 mb-4 shadow-sm transition-colors">
        <div className="flex items-center gap-2 font-bold text-lg text-[var(--text-main)] mb-1">
        ...
        <div className="mt-3 flex items-center gap-3 text-xs text-emerald-500 font-mono font-semibold">
      ...
      {channelMessages.length === 0 ? (
        ...
        channelMessages.map((msg) => {
          ...
          return (
            <div key={msg.id} className="flex gap-3 group hover:bg-[var(--bg-hover)] p-2 rounded-xl transition-colors">
              <Avatar
                src={senderAvatar}
                fallback={senderName}
                status="online"
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="inline-flex items-center gap-0.5 text-[10px] text-[var(--text-muted)] font-medium bg-[var(--bg-surface)] px-1.5 py-0.2 rounded-full border border-[var(--border-subtle)]">
                ...
                {msg.type === 'image' && (
                  <div className="space-y-2 mt-1">
                    <img
                      src={msg.media_url}
                      alt="Attachment"
                      className="max-w-md max-h-72 rounded-xl border border-[var(--border-subtle)] object-cover shadow-md"
                    />
      ...
      {currentTyping.length > 0 && (
        <div className="flex items-center gap-2 text-xs text-[var(--text-muted)] font-mono bg-[var(--bg-card)] border border-[var(--border-subtle)] px-3 py-1.5 rounded-full w-fit animate-pulse">
          <span className="w-2 h-2 rounded-full bg-[var(--text-main)] animate-ping" />
          <span>
